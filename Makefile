...
download_artifacts:
	uv run wandb artifact get tokens_tinystoriesV2_valid_valid:v0
	uv run wandb artifact get tokens_tinystoriesV2_valid_train:v0

train_bpe:
	uv run python cs336_basics/bootstrap.py train_bpe \
		--input_path data/TinyStoriesV2-GPT4-train.txt \
		--out_dir_path data/vocab_tinystoriesV2_train \
		--num_processors 8

tokenize:
	uv run python cs336_basics/bootstrap.py tokenize \
...
train_model:
	uv run python cs336_basics/bootstrap.py train_model \
...
chat:
	uv run python cs336_basics/bootstrap.py chat \
		--checkpoint_path artifacts/checkpoints \
		--vocab_dir_path data/vocab_tinystoriesV2_train \
		--max_gen_len 10 \
		--temperature 0.6 \
...
upload_model:
	uv run wandb artifact put artifacts/checkpoints/*.pt --type model
